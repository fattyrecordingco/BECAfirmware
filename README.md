...
**[Open BECA Phone Control](https://fattyrecordingco.github.io/BECAfirmware/)** on an Android phone in Chrome or Edge, then connect BECA with a USB-C OTG/data cable. The installable web app uses the existing 115200-baud serial protocol to select AUX or Serial + AUX, shape the onboard synth, adjust plant/performance settings, monitor live activity, and send advanced console commands. Its performance views now include a 24-second plant-signal graph with basic signal statistics, the desktop-style 12-leaf MIDI activity/root-note selector, and a two-dimensional cutoff/resonance trackpad for direct AUX sound shaping. After the first load, its application shell is available offline.
