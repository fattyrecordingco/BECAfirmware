...
<html>
  <head>
    ...
    <style>
      ...
      * {
      ...
      body.ui-muted .utility-bar .toggle:not(.mute-toggle):not(.transport-toggle):not(.sync-toggle),
      body.ui-muted #rnd {
        opacity: 0.4;
        pointer-events: none;
      }
  ...
  <body>
        ...
        <div class="synth-grid">
          <div class="subcard module module-cyan">
            ...
            <select id="synthPreset">
              ...
              <option value="1">Prism Poly Lead</option>
              <option value="2">Verdant Pad</option>
              <option value="3">Forest Choir Pad</option>
              <option value="4">Thick Mono Bass</option>
              <option value="5">Rubber Bass</option>
            </select>
    ...
    <div class="utility-bar">
      ...
      <label class="toggle sync-toggle module module-mint">
        ...
        <span class="toggle-val mono" id="syncVal">OFF</span>
      </label>
    ...
    <script>
      ...
      let auxReady = true;
      let auxWaitMs = 0;
      let dawSyncEnabled = false;
      let dawSyncLocked = false;
      ...
      function setDawSyncUi(on, locked = false) {
        dawSyncEnabled = !!on;
        dawSyncLocked = !!locked;
        if (sync) sync.checked = dawSyncEnabled;
        setVal(syncVal, !dawSyncEnabled ? "OFF" : (dawSyncLocked ? "LOCK" : "ARM"));
      }

      async function setDawSyncRemote(on) {
        const prevOn = dawSyncEnabled;
        const prevLocked = dawSyncLocked;
        setDawSyncUi(!!on, false);
        try {
          const res = await postForm("/api/sync", { v: on ? 1 : 0 });
          if (!res.ok) {
            setDawSyncUi(prevOn, prevLocked);
            return;
          }
        } catch (e) {
          setDawSyncUi(prevOn, prevLocked);
      ...
      es.addEventListener("state", (e) => {
        ...
        const dawSyncOn = (j.daw_sync | 0) === 1;
        const dawLockedState = (j.daw_lock | 0) === 1;
        setAuxAvailability(auxReadyState, auxWaitState);
        setDawSyncUi(dawSyncOn, dawLockedState);
        ...
        const clockVal = dawLockedState ? "DAW Lock" : (j.clock ? "Plant" : "Internal");
      ...
      sync.onchange = (e) => setDawSyncRemote(e.target.checked);
